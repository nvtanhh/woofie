...
class PostService extends BaseViewModel {
  ...
  void _onPostEditFailed() {
    injector<ToastService>()
        .error(message: LocaleKeys.save_post_update_failed_toast.trans(), duration: const Duration(seconds: 2), context: Get.context!);
  }
  ...
  void _refreshPost(int postId) {
    call(
      ...
      onSuccess: () {
        injector<ToastService>().success(message: LocaleKeys.save_post_update_success_toast.trans(), context: Get.context!);
        // ignore: invalid_use_of_visible_for_testing_member, invalid_use_of_protected_member
        pagingController.notifyListeners();
      },
      showLoading: false,
